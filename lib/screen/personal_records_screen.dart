...
/// Location: lib/screen/personal_records_screen.dart
library;
...
class _PersonalRecordsScreenState extends State<PersonalRecordsScreen> {
            ...
            children: [
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    const Text('Personal\nRecords', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A), height: 1.1)),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                      decoration: BoxDecoration(color: const Color(0xFF1A1A1A), borderRadius: BorderRadius.circular(20)),
                      child: Text('🏆 ${_records.length} PRs', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
                    ),
                  ],
                ),
              ),
  ...
  Widget _buildEmpty() => Center(
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        Text(
          _searchQuery.isNotEmpty ? 'No results for "$_searchQuery"' : 'No records yet.\nTap + to log your first PR!',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(pr.exercise, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Color(0xFF1A1A1A))),
                const SizedBox(height: 2),
                Text(pr.date, style: const TextStyle(fontSize: 11, color: Colors.black45)),
                if (pr.notes != null && pr.notes!.isNotEmpty) ...[
                  const SizedBox(height: 2),
                  Text(pr.notes!, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, color: Colors.black38, fontStyle: FontStyle.italic)),
                ],
              ],
            ),
          ),
          Text(pr.displayValue, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color(0xFF1A1A1A))),
          const SizedBox(width: 8),
          Column(
            children: [
              GestureDetector(
                onTap: () => _showForm(pr: pr),
                child: const Icon(Icons.edit_outlined, size: 20, color: Colors.black45),
              ),
  ...
  Future<void> _confirmDelete(PersonalRecord pr) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Delete PR', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
        content: Text('Delete your ${pr.exercise} record of ${pr.displayValue}?', style: const TextStyle(color: Colors.white70)),
        actions: [
  ...
  void _showForm({PersonalRecord? pr}) {
    ...
    final exerciseCtrl = TextEditingController(text: isEditing ? pr.exercise : '');
    final valueCtrl = TextEditingController(
      text: isEditing ? (pr.value % 1 == 0 ? pr.value.toInt().toString() : pr.value.toString()) : '',
    );
    final dateCtrl = TextEditingController(text: isEditing ? pr.date : _todayString());
    final notesCtrl = TextEditingController(text: isEditing ? pr.notes ?? '' : '');
    String selectedUnit = isEditing ? pr.unit : _units.first;
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(isEditing ? 'Edit PR' : 'Log New PR 🏆', style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                  const SizedBox(height: 20),
                  _field(controller: exerciseCtrl, label: 'Exercise *', hint: 'e.g. Bench Press, 5km Run', validator: (v) => v == null || v.trim().isEmpty ? 'Enter exercise name' : null),
                  ...
                  const SizedBox(height: 14),
                  _field(controller: dateCtrl, label: 'Date *', hint: 'YYYY-MM-DD', keyboardType: TextInputType.datetime, validator: (v) {
                    if (v == null || v.trim().isEmpty) return 'Enter a date';
                    if (!RegExp(r'^\d{4}-\d{2}-\d{2}$').hasMatch(v.trim())) return 'Use format YYYY-MM-DD';
                    return null;
                  }),
                  const SizedBox(height: 14),
                  _field(controller: notesCtrl, label: 'Notes (optional)', hint: 'e.g. Felt strong, new max'),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(isEditing ? 'Save Changes' : 'Log PR', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700)),
                    ),
                  ),
                ],
              ),
